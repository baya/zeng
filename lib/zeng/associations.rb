  ...
  module Associations
    ...
    class Symmetry
      ...
      def mirror(object)
        ...
        m_o = eval(m_o) if object.is_a? Class
        m_o = m_o.to_sym if object.is_a? Symbol
        m_o
      end

      def each
        asso =  [[eval(@assos[0]), @assos[1].to_sym ],[eval(@assos[-1]), @assos[-2].to_sym]].uniq
    ...
    class << self
      ...
      def constantize_asso_keys(asso)
        ...
        asso.each {|k,v| h[eval(k)] = v.split(' ') }
        h
      end
